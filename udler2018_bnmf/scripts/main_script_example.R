# =============================================================================
# Udler 2018 (PLoS Med) T2D bNMF 재현 — EUR-only 파일럿
# =============================================================================
...
#          num_pops(209행) / filter(n==num_pops)(261행)는 자동으로 따라옴.
#   (사) 295~303행 fetch_summary_stats() — 주석 해제
#        * 원본은 주석 처리돼 있는데 308행이 z_n_mats$df_z 를 쓰므로 그대로 두면
...
# =============================================================================
#
# 원본(BMI multi-ancestry) 버전 이력은 GitHub commit 로그 참조:
...
library(softImpute)
library(strex)
# ---- LOCAL CHANGE: tidyverse 메타패키지 대신 개별 컴포넌트
# (서버에 ragg 시스템 라이브러리 없어 tidyverse 미설치. 개별 컴포넌트는 모두 설치됨) ----
library(tidyr)
library(readr)
library(purrr)
library(tibble)
...
# 로컬: R을 udler2018_bnmf/ 안에서 실행하면 getwd()가 이 폴더를 가리킴.
...
# ---- LOCAL CHANGE: multi-ancestry (원본) → EUR-only (Udler 2018 재현) ----
# 원본: my_pops <- c("EUR","EAS","AFR","AMR","SAS")
# num_pops(209행) 와 filter(n == num_pops)(261~262행)가 여기서 파생되므로
# 193행만 고치면 5개 pop 대신 EUR 하나만 요구하도록 자동 반영됨. 다른 행은 만지지 말 것.
my_pops <- c("EUR")
...
# ---- LOCAL CHANGE:
#   (a) 원본은 이 호출이 주석 처리돼 있는데 아래 zmat0가 z_n_mats를 참조하므로 주석 해제.
#   (b) primary GWAS 를 파일 경로가 아니라 pre-processed 데이터프레임(main_gwas_ready)으로
#       넘김. udler_substitute.R 이 만든 것 — 우리 변환본은 SNP 컬럼이 없어 grep 안 되므로.
#   (c) 존재하지 않는 read_trait_method 인수 제거 (원본 저장소 시그니처에 없음).
# ----
z_n_mats <- fetch_summary_stats(
  df_input       = fetch_input,
  gwas_ss_file   = main_gwas_ready,     # data frame (not file path)
