...
v1(35 trait)과 v2(44 trait)를 모두 실행했다. 두 실행 모두 Udler의 curated 94 SNP를 직접
넣었기 때문에, disease GWAS로부터 SNP를 새로 발견하는 end-to-end 검증이 아니라
downstream matrix+bNMF 검증이다.

폴더는 2026-09-10 에 재편했다: 입력 준비 스크립트는 `tools/`, 문서는 `docs/`,
실행 산출물은 `results/<version>/`, 대용량 입력은 `data/` (git 미포함).

### 입력과 데이터 준비

| 파일 | 목적 |
| --- | --- |
| `udler2018_bnmf/manifest.xlsx` | bNMF가 읽을 disease/trait 파일 경로와 sample size. **44-trait v2** (35 + CHARGE 지방산 5 + GLGC 지질 4). |
| `udler2018_bnmf/inputs_manifest.csv` | trait별 원 데이터 출처, URL, 로컬 경로와 접근 상태를 기록한다. |
| `udler2018_bnmf/tools/download_inputs.sh` | 승인 없이 자동으로 받을 수 있는 summary statistics를 다운로드한다. 소실된 URL이나 승인 자료는 수동 처리가 필요하다. |
| `udler2018_bnmf/tools/build_rsid_map.py` | dbSNP b151 GRCh37 common variant 자료로 rsID-hg19 위치/allele map을 만든다. multi-allelic variant는 allele별로 보존해야 한다. |
| `udler2018_bnmf/tools/convert_sumstats.py` | 서로 다른 원본 형식을 공통 `VAR_ID/BETA/SE/P/N` 형식으로 변환한다. |
| `udler2018_bnmf/tools/validate_inputs.py` | 파일 존재 여부, 헤더, sample size 등 실행 전 조건을 검사한다. runtime 전체 성공을 보장하는 검사는 아니다. |
| `udler2018_bnmf/tools/one_off/` | 이미 적용 완료된 열 추가 스크립트(CHARGE 5, GLGC 4). 기록용. |
| `udler2018_bnmf/refs/hg19ToHg38.over.chain` | 후속 gene annotation 등을 위한 hg19-to-hg38 좌표 변환 파일. |
| `udler2018_bnmf/data/` | 원본·변환본 summary statistics, rsID map, sqlite 색인. 약 8GB, git 미포함. |
...
| `udler2018_bnmf/run_pipeline.R` | 위 단계를 순서대로 묶어 실행하는 드라이버. `Rscript run_pipeline.R` 한 번이면 끝난다. |
...
| `udler2018_bnmf/results/udler2018_eur_v1/` | v1(35 trait)의 W/H matrix, K 분포, alignment 및 heatmap. |
| `udler2018_bnmf/results/udler2018_eur_v2/` | v2(44 trait)의 같은 산출물. 현재 기준 결과. |
| `udler2018_bnmf/reports/` | v2 보고서(docx), 공유용 요약, 정답지 대조 그림과 생성 스크립트. |

T2D v1 결과 (35 trait):

- 입력 trait 35개 -> QC 후 unique trait 26개
- Udler curated SNP 94개 -> 최종 71개
- bNMF의 K=3 선택: 100회 중 96회
- Udler Beta-cell 과 `r=0.92`. 나머지 4개 cluster는 미재현 (Liver/Lipid `r=0.38`)

T2D v2 결과 (44 trait, 현재 기준):

- 입력 trait 44개 -> QC 후 unique trait 31개 (62 feature)
- Udler curated SNP 94개 -> 최종 77개
- bNMF의 K 선택: K=4 70회, K=5 27회 (100회 중)
- Udler 정답지 대조 (형질축 r / 변이축 r) — **5개 중 4개 재현**:
...
v1 과 v2 의 차이는 **입력 열 9개 추가뿐**이고 파이프라인 설정은 같다. Liver/Lipid 가
`r=0.38` -> `0.96` 으로 회복된 것은, v1 의 재현 실패 원인이 파이프라인이 아니라
입력 부족이었음을 보여준다.

### 알려진 한계 및 실패 모드

- v1/v2 모두 `udler_substitute.R`로 SNP를 주입하므로 자체 disease-SNP discovery를
  검증하지 않는다. **`choose_variants_2025.R` 은 한 번도 실행되지 않았다.**
  CD 에는 정답지 SNP 목록이 없어 이 모듈이 반드시 필요하므로, 별도 검증 계획이 필요하다.
- v2 는 proxy 탐색(Section 7)을 의도적으로 건너뛰었다. 그 결과 proxy 대상 23개 SNP 의
  z 값 상당수가 실측이 아니라 softImpute 대치값이고, QC Check 6 이 FAIL 로 뜬다(의도된 것).
- 지질 4개(`hdl` `ldl` `tc` `tg`)는 원 논문의 ENGAGE 2015 배포처가 죽어
  GLGC Willer 2013 으로 대체했다. **출처가 다르므로 결과 대조 시 명시해야 한다.**
...
- Udler curated SNP를 사용한 T2D v1(35 trait) / v2(44 trait) matrix 및 bNMF 실행
- v2 에서 Udler 5개 cluster 중 4개 재현 (Beta-cell, Liver/Lipid, Obesity, Proinsulin)

아직 완료되지 않음:

- `choose_variants_2025.R` (disease-SNP discovery) 검증 — CD 적용의 전제조건
...
2. T2D v1(35 trait) 결과와 v2(44 trait) 결과를 구분한다. 현재 기준은 v2 다.
3. T2D의 회수 검증, 데이터 변환 검증, bNMF 검증은 서로 다른 증거다.
4. v2 의 4개 cluster 재현을 전체 end-to-end 파이프라인 검증으로 확대 해석하지 않는다.
   SNP 선택 단계는 우회했으므로 검증 범위 밖이다.
